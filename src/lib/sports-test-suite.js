...
class SportsScraperTestSuite {
    ...
    async runFullTestSuite() {
        ...
        try {
            ...
            results.totalTests = Object.values(results.details).reduce((sum, test) => sum + test.totalTests, 0);
        ...
        for (const mockPage of mockPlayerPages) {
            ...
            try {
                ...
                testResults.details.push({
                    ...
                    passedRules: Object.entries(validation.results).filter(([_, passed]) => passed).map(([rule, _]) => rule),
    ...
    async testPerformance() {
        ...
        performanceResults.averageExtractionTime = performanceResults.extractionSpeed.reduce((sum, test) => sum + test.timeMs, 0) / performanceResults.extractionSpeed.length;
    ...
    async testEdgeCases() {
        ...
        for (const edgeCase of edgeCases) {
            ...
            try {
                ...
                const handledGracefully = extractionResult.content !== undefined && 
                                        extractionResult.structuredData !== undefined;
    ...
    assessExtractionAccuracy(extractionResult, expected) {
        ...
        if (extractionResult.structuredData.player.name === expected.name) {
            ...
            issues.push(`Name mismatch: expected "${expected.name}", got "${extractionResult.structuredData.player.name}"`);
        }
        ...
        if (extractionResult.structuredData.player.position === expected.position) {
            ...
            issues.push(`Position mismatch: expected "${expected.position}", got "${extractionResult.structuredData.player.position}"`);
        }
    ...
    calculatePlayerDataCompleteness(playerData) {
        const fields = ['name', 'position', 'height', 'weight', 'birthDate', 'birthPlace', 'college', 'draft'];
    ...
    generateRecommendations(results) {
        ...
        if (extractionAccuracy.passed / extractionAccuracy.totalTests < 0.8) {
            recommendations.push({
                priority: 'high',
                category: 'extraction',
                message: 'Content extraction accuracy is below 80%. Review selector patterns and scoring algorithms.'
            });
        }
        ...
        if (structuredDataQuality.passed / structuredDataQuality.totalTests < 0.7) {
            recommendations.push({
                ...
                message: 'Structured data quality is below 70%. Improve biographical data extraction patterns.'
            });
        }
    ...
    printTestResults(results) {
        ...
        Object.entries(results.details).forEach(([category, categoryResults]) => {
            if (categoryResults.totalTests) {
                ...
                console.log(`${category}: ${categoryResults.passed}/${categoryResults.totalTests} (${passRate}%)`);
            }
        });
