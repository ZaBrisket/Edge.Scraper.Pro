...
const TEAM_HISTORY: Record<string, Record<number, TeamInfo>> = {
...
const STATIC_TEAMS: Record<string, TeamInfo> = {
 ...
 */
export function getTeamInfo(pfrCode: string, season: number): TeamInfo {
  // Check if team has historical changes
  if (TEAM_HISTORY[pfrCode]) {
    // Find the appropriate season range
    const years = Object.keys(TEAM_HISTORY[pfrCode]).map(Number).sort((a, b) => a - b);
  ...
  console.warn(`Unknown team code: ${pfrCode} for season ${season}`);
  return {
 ...
 */
export function getMultiTeamInfo(): TeamInfo {
  return {
