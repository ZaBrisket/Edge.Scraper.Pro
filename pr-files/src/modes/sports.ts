...
const SportsInputSchema = z.object({
  ...
  options: z.object({
    ...
    sportsSite: z.enum(['pro-football-reference', 'basketball-reference', 'baseball-reference', 'hockey-reference', 'auto']).optional().default('auto'),
  }).optional().default({}),
});
...
export class SportsMode implements ModeContract {
  ...
  public readonly description = 'Extract player statistics, biographical data, and achievements from sports reference sites';
  ...
  public readonly uiHints = {
    ...
    maxBatchSize: 200,
    fileFormats: ['txt', 'csv'],
    placeholder: 'Enter sports player URLs (one per line)\nExample: https://www.pro-football-reference.com/players/M/MahoPa00.htm',
    helpText: 'Extracts player statistics, biographical information, and career achievements from Pro Football Reference and other sports sites.',
    examples: [
      ...
      const modeResult = {
        results: batchResult.results.map(result => ({
          ...
          data: result.success && result.data ? {
            ...
            statistics: input.options?.extractTables !== false ? result.data.statistics : undefined,
  ...
  async validate(input: any): Promise<{ valid: boolean; errors?: string[] }> {
    ...
    // Validate options
    if (input.options) {
      if (input.options.concurrency && (input.options.concurrency < 1 || input.options.concurrency > 5)) {
