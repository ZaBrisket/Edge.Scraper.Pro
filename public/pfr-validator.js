/**
 * PFR (Pro Football Reference) URL Validator Module (Browser Version)
 * 
...
class PFRValidator {
  ...
  generateHTMLReport(batchResult) {
    ...
    if (invalidCategories.length > 0) {
      ...
      invalidCategories.forEach(([category, urls]) => {
        const categoryName = category.replace(/_/g, ' ').charAt(0).toUpperCase() + 
        ...
        urls.forEach(({ url, error }) => {
          html.push(`<li>`);
          html.push(`<span class="invalid-url">${url}</span>`);
          if (error) html.push(`<br><small style="color: #666">Error: ${error}</small>`);
          html.push(`</li>`);
        });
...
window.PFRValidator = PFRValidator;
window.VALIDATION_CATEGORIES = VALIDATION_CATEGORIES;