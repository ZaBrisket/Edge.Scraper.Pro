...
export default function NdaPage() {
  ...
  const [proposedDocx, setProposedDocx] = useState<Uint8Array | null>(null);
  const [consented, setConsented] = useState(false);
  const [useAspose, setUseAspose] = useState(false);
  const [loading, setLoading] = useState(false);

  useEffect(() => {
    if (typeof window !== 'undefined' && process.env.NEXT_PUBLIC_POSTHOG_KEY && !(posthog as any).__loaded) {
  ...
  async function runReview() {
    ...
    try {
      ...
      const payload = {
        file: {
          content: toBase64(new Uint8Array(content)),
          contentType: file.type || (file.name.endsWith('.pdf') ? 'application/pdf' : 'application/octet-stream')
        },
        options: { useOCR }
      };
      const res = await fetch('/api/nda/review', { method: 'POST', body: JSON.stringify(payload) });
      if (res.status === 412) { alert('Scanned PDF requires OCR. Try Server Assist or upload a text-native PDF.'); return; }
      if (res.status === 503) { alert('Service temporarily unavailable. Please try again later.'); return; }
      if (res.status === 429) { const d = await res.json(); alert(`Rate limit exceeded. Try again in ${d.reset} seconds.`); return; }
  ...
  async function exportDocx() {
    ...
    setLoading(true);
    try {
      const originalArr = new Uint8Array(await file.arrayBuffer());
      let res = await fetch('/api/nda/export-docx', {
        method: 'POST',
        body: JSON.stringify({
          originalBase64: toBase64(originalArr),
          proposedDocxBase64: toBase64(proposedDocx),
          resultFileName: `nda_redlines_${Date.now()}.docx`
        })
      });
      if (res.status === 503) { alert('Aspose unavailable, using basic export...'); await exportBasic(); return; }
      if (!res.ok) { alert('Export failed'); return; }
      const blob = await res.blob();
      const url = URL.createObjectURL(blob);
      const a = Object.assign(document.createElement('a'), { href: url, download: 'nda_redlines.docx' });
  ...
  async function exportBasic() {
    ...
    setLoading(true);
    try {
      const text = result?.findings?.map((f: any) =>
        `${f.clause}: ${f.status}\n${f.rationale || ''}\n${f.notes?.[0] || ''}`
      ).join('\n\n') || '';
      ...
      const blob = await res.blob();
      const url = URL.createObjectURL(blob);
      const a = Object.assign(document.createElement('a'), { href: url, download: 'nda_redlines_basic.docx' });
      a.click(); URL.revokeObjectURL(url);
  ...
  async function capture(event: string, props: any) {
    try {
      ...
      await fetch('/api/telemetry', { method: 'POST', body: JSON.stringify({ event, props }) }).catch(() => {});
    } catch {}
  }
  ...
  return (
    <main className="container mx-auto px-4 py-6 max-w-6xl">
      ...
      <p className="text-sm text-gray-600 mb-4">
        This tool provides automated checklist analysis only. It is <b>NOT legal advice</b>. All exports are watermarked.
        You must confirm consent before processing.
      </p>
      ...
      <div className="bg-white shadow rounded-lg p-6 mb-6">
        ...
        <input type="file" accept=".pdf,.docx,.txt" onChange={(e) => setFile(e.target.files?.[0] ?? null)} className="mb-3" />
        <div className="flex gap-4 items-center mb-4">
          ...
          <label className="flex items-center gap-2">
            <input type="checkbox" checked={useAspose} onChange={(e) => setUseAspose(e.target.checked)} />
      ...
      {result && (
        <>
          <div className="grid md:grid-cols-2 gap-4 mb-6">
            <div className="bg-white shadow rounded-lg p-6">
                ...
                <table className="w-full text-sm">
                  ...
                  <tbody className="divide-y">
                    {result.findings.map((f: any) => (
                      <tr key={f.clause} className={f.status === 'FAIL' ? 'bg-red-50' : f.status === 'WARN' ? 'bg-yellow-50' : 'bg-green-50'}>
              ...
              <div className="mt-4 text-xs text-gray-600">
                Version: {result.checklistVersion} | Variant: {result.variant} | Pages: {result.stats.pages || 'N/A'}
              </div>
            </div>
            <div className="bg-white shadow rounded-lg p-6">
              ...
              <div className="space-y-3 overflow-auto max-h-96">
                {result.findings.slice(0, 5).map((f: any) => (
                  <div key={f.clause} className="border-l-4 border-gray-300 pl-3">
                    ...
                    {f.evidence?.[0]?.text && <pre className="bg-gray-50 p-2 text-xs whitespace-pre-wrap mt-1">{f.evidence[0].text.slice(0, 200)}...</pre>}
                  </div>
                ))}
              </div>
            </div>
          ...
          <div className="bg-white shadow rounded-lg p-6">
            ...
            {useAspose && (
              <div className="mb-4">
                <p className="text-sm text-gray-600 mb-2">Upload revised DOCX to generate tracked changes:</p>
                <input type="file" accept=".docx" onChange={(e) => e.target.files?.[0]?.arrayBuffer().then((b) => setProposedDocx(new Uint8Array(b)))} />
                <button
            ...
            <button className="px-3 py-1 bg-gray-600 text-white rounded hover:bg-gray-700 disabled:opacity-50" onClick={exportBasic} disabled={!result || !consented || loading}>
              Export Basic DOCX
            </button>
          </div>
        </>
