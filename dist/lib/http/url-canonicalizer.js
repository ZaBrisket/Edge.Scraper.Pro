...
class UrlCanonicalizer {
    ...
    try {
      ...
      const baseVariants = [
        ...
        protocol === 'http:' && !hostname.startsWith('www.')
          ? `https://www.${hostname}${normalizedPath}${search}${hash}`
          : null,
        ...
        protocol === 'http:' && !hostname.startsWith('www.')
          ? `https://www.${hostname}${pathWithSlash}${search}${hash}`
          : null,

        // 6. Remove www if present (apex domain)
        hostname.startsWith('www.')
          ? `${protocol}//${hostname.substring(4)}${normalizedPath}${search}${hash}`
          : null,

        // 7. HTTPS + remove www
        protocol === 'http:' && hostname.startsWith('www.')
          ? `https://${hostname.substring(4)}${normalizedPath}${search}${hash}`
          : null,
      ];
  ...
  async canonicalizeUrl(originalUrl, fetchClient) {
    ...
    if (cached && Date.now() - cached.timestamp < this.cacheMaxAge) {
      this.logger.debug(
        { originalUrl, canonicalUrl: cached.result.canonicalUrl },
        'Using cached canonicalization'
      );
    ...
    for (let i = 0; i < variants.length && i < this.options.maxVariants; i++) {
      ...
      if (
        i > 0 &&
        this.options.backoffDelays[Math.min(i - 1, this.options.backoffDelays.length - 1)]
      ) {
        const delay =
          this.options.backoffDelays[Math.min(i - 1, this.options.backoffDelays.length - 1)];
  ...
  cleanupCache() {
    ...
    if (cleaned > 0) {
      this.logger.debug(
        { cleaned, remaining: this.canonicalCache.size },
